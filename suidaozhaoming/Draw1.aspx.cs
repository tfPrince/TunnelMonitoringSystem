...
{
    public partial class Draw1 : System.Web.UI.Page
    {
        ...
        private void SetBind()
        {
            ...
            {
                case "0100000000000":
                    if (Session["IntoCode"].ToString() == "Come from Button Left")
                    {
                        String sqlconnection1 = ConfigurationManager.ConnectionStrings["suidaozhaomingConnectionString3"].ToString();
                        String Sqlempinfo = "select top(1) starttime,endtime from LightParam where ( cuid = 'CC6012070168')and (code = 'Page27') order by id desc";
                    ...
                    break;
                case "0200000000000":
                    if (Session["IntoCode"].ToString() == "Come from Button Right")
                    {
                        String sqlconnection1 = ConfigurationManager.ConnectionStrings["suidaozhaomingConnectionString3"].ToString();
                        String Sqlempinfo = "select top(1) starttime,endtime from LightParam where ( cuid = 'CC6012071120')and (code = 'Page27') order by id desc";
                        SqlConnection conn1 = new SqlConnection(sqlconnection1);
                        SqlCommand lightinfo = new SqlCommand(Sqlempinfo, conn1);
                        conn1.Open();
                        SqlDataReader light_info = lightinfo.ExecuteReader();
                ...
                case "0300000000000":
                    ...
                    {
                        String sqlconnection1 = ConfigurationManager.ConnectionStrings["suidaozhaomingConnectionString3"].ToString();
                        String Sqlempinfo = "select top(1) starttime,endtime from LightParam where ( cuid = 'CC0000000000')and (code = 'Page27') order by id desc";
                        SqlConnection conn1 = new SqlConnection(sqlconnection1);
                        SqlCommand lightinfo = new SqlCommand(Sqlempinfo, conn1);
                        conn1.Open();
                        SqlDataReader light_info = lightinfo.ExecuteReader();
            ...
            switch (Session["suidaocode"].ToString())
            {
                case "0100000000000":
                    ...
                    //String SqlLightDisData = "select lightness,dbtime from LightLevel where(( cuid = 'CC6012070168') and (dbtime between '"+day1+"' and '"+day2+"')) order by dbtime desc";
                ...
                case "0200000000000":
                    if (Session["IntoCode"].ToString() == "Come from Button Left")
                    {
                        ...
                        try
                        {
                            //string strData = "DATA" + this.lbTt.Text;
                            //初始化所建表的SQL语句,属于新闻类别
                            this.lbTt.Text = "右洞";
                            //DataSet ds = new DataSet();
                            //MyAdapter.Fill(ds, "LightFeel");
                    ...
                    {
                        String sqlconnection = ConfigurationManager.ConnectionStrings["suidaozhaomingConnectionString3"].ToString();
                        String SqlLightDisData = "select lightness,dbtime from LightLevel where(( cuid = 'CC6012071120') and (dbtime between '" + day1 + "' and '" + day2 + "')) order by dbtime desc";
                        SqlConnection conn = new SqlConnection(sqlconnection);
                        SqlDataAdapter MyAdapter = new SqlDataAdapter(SqlLightDisData, conn);  
                        if (Session["IntoCode"].ToString() == "Come from Button Right")
                        {
                ...
                case "0300000000000":
                    ...
                    {
                        String sqlconnection = ConfigurationManager.ConnectionStrings["suidaozhaomingConnectionString3"].ToString();
                        String SqlLightDisData = "select VIness,dbTime from lightData where((tunnelCode = '0300000000000') and (dbtime between '" + day1 + "' and '" + day2 + "')) order by dbTime desc";
        ...
        public static void draw(Page page, DataSet ds, int Tnum, string strName)
        {
            try
            {
                ...
                string info = strName.Replace("'", "''").Trim() + "亮度曲线图生成时间：" + Convert.ToDateTime(ds.Tables[0].Rows[0]["dbtime"]).ToString() + "  记录数据：" + count;   // + "  当前探头：" + strName.Replace("'", "''").Trim() + "#"
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    for (int l = j; l < k; l++)
                    {
                        ...
                        //    valueCar += Convert.ToInt16(ds.Tables[0].Rows[Convert.ToByte(l)][1]);
                    }
                    value = value / (k - j);
                    ...
                    //绘制车流量值记录点
                    //g.DrawString(valueCar.ToString(), font, Greenbrush, p[i].X, pCar[i].Y - 10);
                    //g.DrawRectangle(Bp, p[i].X - 1, pCar[i].Y - 1, 2, 2);
