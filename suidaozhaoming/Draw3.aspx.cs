...
{
    public partial class Draw3 : System.Web.UI.Page
    {
        ...
        public static void draw(Page page, DataSet ds, int Tnum, string strName)
        {
            try
            {
                ...
                int xSpace = (wd - 100) / (countt - 1);

                //定义纵坐标间隔,不能随便修改，跟高度和横坐标线的条数有关，最佳值=(绘图的高度-上面留空-下面留空)
                int ySpace = 12;
                //纵坐标最大值和间隔值
                int yMaxValue = 10;               //纵坐标标注最大值
                //绘制底色
                ...
                //定义曲线转折点
                Point[] pEnergy = new Point[count-1];
                Double valueCar = 0;
                for (int i = 0; i < count-1; i++)
                {
                    ...
                    if (strName == "每小时")
                    {
                        ...
                        valueCar = Convert.ToDouble(ds.Tables[0].Rows[i][0]) - Convert.ToDouble(ds.Tables[0].Rows[i + 1][0]);
                        pEnergy[i].Y = 360 - Convert.ToInt16(valueCar * 1000 / (yAveValue));
                    }
                    else if (strName == "每天")
                    {
                        pEnergy[i].X = 40 + xSpace * i;
