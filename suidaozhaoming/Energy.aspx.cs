        ...
        protected void update(DateTime day1, DateTime day2)
        {
            switch (Session["suidaocode"].ToString())
            {
                case "0100000000000":
                    ...
                    String Sqlempinfo = "select top(1) id from LightParam where ((cuid = 'CC6012070168')and( code = 'Energy')) order by id desc";
                    ...
                    if (light_info.Read())
                    {
                        conn.Close();
                        //String sqlconnection = ConfigurationManager.ConnectionStrings["suidaozhaomingConnectionString3"].ToString();
                        Sqlempinfo = "update LightParam set starttime = '" + day1 + "' ,endtime= '" + day2 + "'  where ((cuid = 'CC6012070168')and(code ='Energy')) ";
                        //SqlConnection conn = new SqlConnection(sqlconnection);
                    ...
                    else
                    {
                        conn.Close();
                        //SqlConnection conn = new SqlConnection(sqlconnection);
                        String Sqlempinfosert = "insert into LightParam values( 'CC6012070168','" + day1 + "', '" + day2 + "','Energy')";
